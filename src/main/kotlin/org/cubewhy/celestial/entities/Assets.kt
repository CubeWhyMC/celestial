...
class Assets {
    ...
    override fun equals(o: Any?): Boolean {
        ...
        if (if (`this$browser_download_url` == null) `other$browser_download_url` != null else `this$browser_download_url` != `other$browser_download_url`) return false
        return true
    }
    ...
    override fun hashCode(): Int {
        ...
        result = result * PRIME + (`$uploader`?.hashCode() ?: 43)
        val `$content_type`: Any? = this.content_type
        result = result * PRIME + (`$content_type`?.hashCode() ?: 43)
        val `$state`: Any? = this.state
        result = result * PRIME + (`$state`?.hashCode() ?: 43)
        val `$size` = this.size
        ...
        result = result * PRIME + (`$updated_at`?.hashCode() ?: 43)
        val `$browser_download_url`: Any? = this.browser_download_url
        result = result * PRIME + (`$browser_download_url`?.hashCode() ?: 43)
        return result
    }
